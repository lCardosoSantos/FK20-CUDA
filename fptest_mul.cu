...
/**
 * @brief Test for the commutative property of addition using PTX macros
 * 
 * x*y == y*x
 * 
 * @param testval 
 * @return void 
 */
__global__ void FpTestCommutativeMulPTX(testval_t *testval) {
    ...
    for (int i=0; i<TESTVALS; i++) {
        for (int j=0; j<TESTVALS; j++) {
            ...
            fp_mul_ptx(x, x, testval[j]);  // x * y
            fp_mul_ptx(y, y, testval[i]);  // y * x
...
/**
 * @brief Test for the associative property of multiplication using PTX macros
 * 
 * (x*y)*z == x*(y*z)
 * 
 * @param testval 
 * @return void 
 */
__global__ void FpTestAssociativeMulPTX(testval_t *testval) {
    ...
    for (int i=0; i<TESTVALS; i++) {
        for (int j=0; j<TESTVALS; j++) {
            for (int k=0; k<TESTVALS; k++) {
                ...
                fp_mul_ptx(a, a, testval[j]);  // x * y
                fp_mul_ptx(a, a, testval[k]);  // (x * y) * z

                fp_mul_ptx(b, b, testval[k]);  // y * z
                fp_mul_ptx(c, c, b);           // x * (y * z)
...
/**
 * @brief Multiplication test, using different values for different threads. using PTX macros
 * 
 * 
 * @param testval 
 * @return void 
 */
__global__ void FpTestMulPTX(testval_t *testval) {
