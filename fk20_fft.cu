...
__global__ void g1p_fft(g1p_t *output, const g1p_t *input) {
    ...
    //g1p_mul(g1p[r], fr_roots[w]);
    g1p_addsub(g1p[l], g1p[r]);
...
__global__ void g1p_ift(g1p_t *output, const g1p_t *input) {
    ...
    g1p_addsub(g1p[l], g1p[r]);
    g1p_mul(g1p[l], fr_roots[513]);    // 2**-9
    g1p_mul(g1p[r], fr_roots[513+w]);  // w ? 2**-9/fr_roots[128] : 2**-9
