...
#define NEGPRINTPASS(pass)                                                                                                \
    printf("- %s\n", pass ? COLOR_RED COLOR_BOLD "FALSIFIABILITY FAIL" COLOR_RESET : COLOR_GREEN "FALSIFIABILITY PASS" COLOR_RESET);
...
#define CLOCKEND                                                                                                       \
    ...
    printf(" (%.1f ms) ", (end - start) * (1000. / CLOCKS_PER_SEC))

// Check if the array generated by the fr_eq() and g1p_eq() kernels have a failure, then report.
#define CMPCHECK(LENGHT)                                                                                               \
    for (int i = 0; pass && i < LENGHT; i++) {                                                                         \
        if (cmp[i] != 1) {                                                                                             \
