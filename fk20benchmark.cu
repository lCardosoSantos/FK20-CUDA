...
// known valid values generated by the python implementation
...
fr_t  *b_fr_tmp;
g1p_t *b_g1p_tmp;
__managed__ uint8_t cmp[16*512]; //Comparison array in GPU
...
// Synchronizes the Device, making sure that the kernel has finished the execution. Checks for any errors, and report if
// errors are found.
#define CUDASYNC(fmt, ...)                                                                                             \
...
int main(int argc, char **argv) {
    ...
    if (!pass) {
        // Might be interesting some times to have the benchmark run even if the
 ...
 * if the calculated h_fft is the same as the KAT. This function does not replace
 * the more in depth tests, but works as a Canary to detect errors. 
 * 
 ...
 * This is the closest we have to the real-world performance.
 * 
 * @param rows number of rows and cuda blocks
 */
void benchFull(int rows){
